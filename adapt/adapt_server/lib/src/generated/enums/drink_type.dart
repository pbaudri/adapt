/* AUTOMATICALLY GENERATED CODE DO NOT MODIFY */
/*   To generate run: "serverpod generate"    */

// ignore_for_file: implementation_imports
// ignore_for_file: library_private_types_in_public_api
// ignore_for_file: non_constant_identifier_names
// ignore_for_file: public_member_api_docs
// ignore_for_file: type_literal_in_constant_pattern
// ignore_for_file: use_super_parameters
// ignore_for_file: invalid_use_of_internal_member

// ignore_for_file: no_leading_underscores_for_library_prefixes
import 'package:serverpod/serverpod.dart' as _i1;

enum DrinkType implements _i1.SerializableModel {
  beer,
  wine,
  champagne,
  cocktail,
  spirit,
  shooter,
  liqueur,
  longDrink,
  hardSeltzer,
  other;

  static DrinkType fromJson(String name) {
    switch (name) {
      case 'beer':
        return DrinkType.beer;
      case 'wine':
        return DrinkType.wine;
      case 'champagne':
        return DrinkType.champagne;
      case 'cocktail':
        return DrinkType.cocktail;
      case 'spirit':
        return DrinkType.spirit;
      case 'shooter':
        return DrinkType.shooter;
      case 'liqueur':
        return DrinkType.liqueur;
      case 'longDrink':
        return DrinkType.longDrink;
      case 'hardSeltzer':
        return DrinkType.hardSeltzer;
      case 'other':
        return DrinkType.other;
      default:
        throw ArgumentError('Value "$name" cannot be converted to "DrinkType"');
    }
  }

  @override
  String toJson() => name;

  @override
  String toString() => name;
}
